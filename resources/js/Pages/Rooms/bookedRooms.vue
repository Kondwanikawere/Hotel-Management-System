...
watch(search, debounce(
        (q) => {
            ...
            form.family =  null;
            form.executive =  null;
            router.get('/bookedRooms', {search: q}, {preserveState: true})
        },
        500
    )
)
    ...
    <dashboardHeader :content="content">
        <template #main>
            <div class="h-[650px] flex flex-col items-center w-full bg-white text-black pt-[15px] divide-y-2 border-hBackground font-[Poppins]">
                <p class="text-[18px] pb-[10px]">Booked rooms</p>
                            ...
                            <Menu as="div">
                                <div>
                                    <MenuButton class="inline-flex w-full justify-center">
                                        <img src="/public/images/Filter.png" alt="filter" class="w-[30px] h-[30px]">
                                    </MenuButton>
                                </div>
                        ...
                        <table class="flex flex-col w-[100%] border-2 border-hBackground divide-y-2 divide-hBackground">
                            <thead class="">
                                <tr class="flex flex-row  h-[50px] w-[100%] divide-x-2 divide-hBackground">
                                    <th class="flex items-center justify-center w-[20%]">Room number</th>
                                    <th class="flex items-center justify-center w-[20%]"><p>Type</p></th>
                                    <th class="flex items-center justify-center w-[20%]">Status</th>
                                    <th class="flex items-center justify-center w-[20%]">CustomerId</th>
                                    <th class="flex items-center justify-center w-[20%]">Booked_at</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y-2 divide-hBackground">
                                <tr v-for="room in rooms.data" :key="room.id" class="flex flex-row  h-[60px] w-[100%] divide-x-2 divide-hBackground">
                                    <td class="flex items-center justify-center w-[20%]">{{ room.id }}</td>
                                    <td class="flex items-center justify-center w-[20%]">{{ getRoomType(room.type) }}</td>
                                    <td class="flex items-center justify-center w-[20%]" >
                                        <div class="flex justify-center w-[35%] h-[37px] text-white items-center rounded-[5px] text-[13px]" :class="{'bg-[#02703B]' : room.status === 0, 'bg-[#B00101]' : room.status === 1}">{{ getRoomStatus(room.status) }}</div>
                                    </td>
                                    <td class="flex flex-col items-center justify-center w-[20%]">
                                        {{ room.latest_customer_visit.customerId }}
                                        <Link class="text-[#B00101]" :href="`/customerDetails/${room.latest_customer_visit.customerId}`">
                                            All details
                                        </Link>
                                    </td>
                                    <td class="flex items-center justify-center w-[20%] text-[12px]">{{ getDate(room.latest_customer_visit.checkIn) }}</td>
                                </tr>  
                            </tbody>
                        </table>
                        <div class="pt-2">
