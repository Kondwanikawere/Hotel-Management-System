...
watch(search, debounce(
        (q) => {
            ...
            form.family =  null;
            form.executive =  null;
            router.get('/vacantRooms', {search: q}, {preserveState: true})
        },
        500
    )
)
    ...
    <dashboardHeader :content="content">
        <template #main>
            <div class="h-[650px] flex flex-col items-center w-full bg-white text-black pt-[15px] divide-y-2 border-hBackground font-[Poppins]">
                <p class="text-[18px] pb-[10px]">Vacant rooms</p>
                        ...
                        <table class="flex flex-col w-[100%] border-2 border-hBackground divide-y-2 divide-hBackground">
                            <thead class="">
                                <tr class="flex flex-row  h-[50px] w-[100%] divide-x-2 divide-hBackground">
                                    ...
                                    <th class="flex items-center justify-center w-[25%]">Last booked</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y-2 divide-hBackground">
                                <tr v-for="room in rooms.data" :key="room.id" class="flex flex-row  h-[35px] w-[100%] divide-x-2 divide-hBackground">
                                    ...
                                    <td class="flex items-center justify-center w-[25%] text-[12px]">
                                        <div v-if="room.latest_customer_visit !== null">
                                            {{ getDate(room.latest_customer_visit.checkIn) }}
                                        </div>
                                    </td>
                                </tr>  
                            </tbody>
